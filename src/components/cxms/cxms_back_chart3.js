...
class areaChart {
	constructor(dom) {
		...
		this.linearColors = ['rgba(183,58,66,1)','rgba(128,178,231,1)','rgba(67,207,110,1)','rgba(4,220,253,1)','rgba(41,139,238,1)'];
	}
	...
	creationContent() {
		...
		for (let i = 0; i < len; i++) {
			...
			let rgbaColor = _this.linearColors[i];
			let json = {
				name: item.seriesName,
				type:'line',
				areaStyle:{
					normal:{
						color:{
							...
							colorStops:[{
								offset:0,
								color:rgbaColor.substring(0,rgbaColor.lastIndexOf(',')+1)+'1)'
							},{
								offset:0.3,
								color:rgbaColor.substring(0,rgbaColor.lastIndexOf(',')+1)+'0.2)'
							},{
								offset:1,
								color:rgbaColor.substring(0,rgbaColor.lastIndexOf(',')+1)+'0)'
							}],
							globalCoord:false
						}
					}
